...
def create_bill(data):
    ...
    discount = round(subtotal * 0.05, 2) if subtotal > 1000 else 0.0
...
def report(data):
    ...
    if product_sold:
        pid, qty = sorted(product_sold.items(), key=lambda x: x[1], reverse=True)[0]
        pname = data["products"].get(pid, {}).get("name", "(deleted)")
        print(f"Best selling: {pid} {pname} — {qty} units")
